...
import {
  ...
  Wrench, Filter, Loader2, ClipboardList, LogOut,
} from "lucide-react";
...
export default function CRMInterventions() {
  ...
  if (session === undefined || (session && loading)) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Loader2 className="w-6 h-6 animate-spin text-[#3B6E8F]" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      <header className="bg-[#1C2321] text-[#F5F3EE] sticky top-0 z-20 shadow-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="ticket-font text-xl font-bold leading-none uppercase">Fiches d'intervention</h1>
              <p className="text-[11px] text-[#9CB4C2] mt-0.5">Plomberie · Serrurerie · Vitrerie</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowSettings(true)} className="p-2 rounded-md hover:bg-white/10" aria-label="Paramètres">
              <Settings className="w-5 h-5" />
            </button>
            <button onClick={handleLogout} className="p-2 rounded-md hover:bg-white/10" aria-label="Déconnexion">
              <LogOut className="w-5 h-5" />
            </button>
            <button onClick={openNewForm} className="flex items-center gap-1.5 bg-[#E8A33D] text-[#1C2321] font-semibold px-3 py-2 rounded-md hover:brightness-105 text-sm">
      ...
      <main className="max-w-5xl mx-auto px-4 py-5 pb-24">
        ...
        <div className="mb-5 bg-white rounded-lg border border-[#E3DFD3] shadow-sm">
          <button onClick={() => setShowFilters((v) => !v)} className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium">
            <span className="flex items-center gap-2">
              <Filter className="w-4 h-4 text-[#3B6E8F]" /> Filtres
              {activeFilterCount > 0 && <span className="bg-[#3B6E8F] text-white text-[11px] px-1.5 py-0.5 rounded-full">{activeFilterCount}</span>}
            </span>
            ...
            <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              ...
              {activeFilterCount > 0 && (
                <button
                  ...
                  className="text-sm text-[#3B6E8F] underline self-end mb-1"
                >
                  Réinitialiser
                </button>
              )}
        ...
        {aFinirList.length > 0 && (
          <section className="mb-6">
            ...
            <div className="space-y-2">
              {aFinirList.map((item) => (
                <InterventionCard key={item.id} item={item} onEdit={() => openEditForm(item)} onDelete={() => deleteIntervention(item.id)}
        ...
        {groupedByMonth.map(([key, items]) => {
          ...
          return (
            <section key={key} className="mb-3 bg-white rounded-lg border border-[#E3DFD3] shadow-sm overflow-hidden">
              <button onClick={() => toggleMonth(key)} className="w-full flex items-center justify-between px-4 py-3">
                ...
                <span className="mono-font text-sm font-semibold text-[#3B6E8F]">{formatMontant(total)}</span>
              </button>
              {isOpen && (
      ...
      {showForm && (
        <Modal onClose={closeForm} title={form.id ? "Modifier l'intervention" : "Nouvelle intervention"}>
          {formErrors.length > 0 && (
            <div className="mb-4 flex items-start gap-2 bg-[#C1495F]/10 border border-[#C1495F]/30 text-[#8f3247] px-3 py-2 rounded-md text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="N° devis *" error={formErrors.includes("num_devis")}>
            ...
            <div className="sm:col-span-2">
              ...
              <label className="flex items-center gap-2 justify-center border-2 border-dashed border-[#D8D3C4] rounded-md py-3 cursor-pointer hover:border-[#3B6E8F] text-sm text-[#5c584d]">
              ...
              {attachWarning && <p className="text-xs text-[#C1495F] mt-1">{attachWarning}</p>}
              {(form.attachments || []).length > 0 && (
                <ul className="mt-2 space-y-1">
                  {form.attachments.map((a, idx) => (
                    <li key={idx} className="flex items-center justify-between bg-[#F5F3EE] rounded px-2 py-1.5 text-xs">
                      <span className="flex items-center gap-1.5 truncate">
                        {a.type?.includes("pdf") ? <FileText className="w-3.5 h-3.5 shrink-0" /> : <ImageIcon className="w-3.5 h-3.5 shrink-0" />}
                        <span className="truncate">{a.name}</span>
                      </span>
                      <button onClick={() => removeSavedAttachment(idx)} className="text-[#C1495F] shrink-0 ml-2"><X className="w-3.5 h-3.5" /></button>
                    </li>
                  ))}
                </ul>
              )}
                ...
                <ul className="mt-2 space-y-1">
                  {pendingFiles.map((f, idx) => (
                    <li key={idx} className="flex items-center justify-between bg-[#EDF3F6] rounded px-2 py-1.5 text-xs">
                      ...
                      <button onClick={() => removePendingFile(idx)} className="text-[#C1495F] shrink-0 ml-2"><X className="w-3.5 h-3.5" /></button>
                    </li>
                  ))}
                </ul>
              )}
          ...
          <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-[#E3DFD3]">
            <button onClick={closeForm} className="px-4 py-2 rounded-md text-sm font-medium text-[#5c584d] hover:bg-[#F5F3EE]">Annuler</button>
            <button onClick={saveForm} disabled={saving} className="px-4 py-2 rounded-md text-sm font-semibold bg-[#3B6E8F] text-white hover:brightness-110 flex items-center gap-2 disabled:opacity-60">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <div className="flex gap-2 mb-2">
                ...
                <button onClick={addTechnicien} className="px-3 rounded-md bg-[#3B6E8F] text-white"><Plus className="w-4 h-4" /></button>
              </div>
              <ul className="space-y-1">
                {techniciens.map((t) => (
                  <li key={t} className="flex items-center justify-between bg-[#F5F3EE] rounded px-2 py-1.5 text-sm">
                    {t}
                    <button onClick={() => removeTechnicien(t)} className="text-[#C1495F]"><Trash2 className="w-3.5 h-3.5" /></button>
                  </li>
                ))}
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                ...
                <button onClick={addAgence} className="px-3 rounded-md bg-[#3B6E8F] text-white"><Plus className="w-4 h-4" /></button>
              </div>
              <ul className="space-y-1">
                {agences.map((a) => (
                  <li key={a} className="flex items-center justify-between bg-[#F5F3EE] rounded px-2 py-1.5 text-sm">
                    {a}
                    <button onClick={() => removeAgence(a)} className="text-[#C1495F]"><Trash2 className="w-3.5 h-3.5" /></button>
                  </li>
                ))}
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {atts.map((a, idx) => (
                  <a key={idx} href={a.url} target="_blank" rel="noreferrer" className="border border-[#E3DFD3] rounded-md p-2 flex flex-col items-center gap-1.5 hover:border-[#3B6E8F]">
                    {a.type?.startsWith("image/") ? (
                      <img src={a.url} alt={a.name} className="w-full h-20 object-cover rounded" />
                    ) : (
                      <FileText className="w-8 h-8 text-[#3B6E8F]" />
                    )}
...
function Field({ label, children, error, span2 }) {
  return (
    <div className={span2 ? "sm:col-span-2" : ""}>
      {label && <label className={`text-xs font-medium block mb-1 ${error ? "text-[#C1495F]" : "text-[#5c584d]"}`}>{label}</label>}
      {children}
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start sm:items-center justify-center z-30 p-3 overflow-y-auto">
      <div className="bg-white rounded-lg max-w-2xl w-full my-6 shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E3DFD3] sticky top-0 bg-white rounded-t-lg">
          <h2 className="ticket-font uppercase text-lg font-bold">{title}</h2>
          <button onClick={onClose} className="p-1 rounded hover:bg-[#F5F3EE]"><X className="w-5 h-5" /></button>
        </div>
...
function InterventionCard({ item, onEdit, onDelete, onTogglePayee, onToggleAFinir, onViewAttachments }) {
  const attente = item.type_reglement === "En attente de règlement" && !item.payee;
  let borderColor = "#E3DFD3";
  let bg = "white";
  if (item.a_finir) { borderColor = "#E8A33D"; bg = "#FDF6E8"; }
  else if (attente) { borderColor = "#C1495F"; bg = "#FBEDEF"; }

  const dateFmt = item.date_intervention ? new Date(item.date_intervention).toLocaleDateString("fr-FR") : "—";

  return (
    <div className="rounded-lg border-l-4 shadow-sm px-3 py-2.5" style={{ borderLeftColor: borderColor, background: bg, borderTop: "1px solid #E3DFD3", borderRight: "1px solid #E3DFD3", borderBottom: "1px solid #E3DFD3" }}>
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            <span className="font-semibold text-sm truncate">{item.client_nom}</span>
            <span className="text-[11px] px-1.5 py-0.5 rounded bg-[#EDEAE0] text-[#5c584d]">{item.type_intervention}</span>
            <span className="text-[11px] text-[#8b8677]">{item.mission}</span>
          </div>
          <p className="text-xs text-[#8b8677] mt-0.5 truncate">
            {item.client_adresse}, {item.client_cp} {item.client_ville} · {item.client_tel}
          </p>
          <p className="text-xs text-[#5c584d] mt-1">{dateFmt} · {item.technicien} · {item.agence}</p>
          <p className="mono-font text-[11px] text-[#8b8677] mt-1">Devis {item.num_devis} · Facture {item.num_facture} · {item.type_reglement}</p>
          {item.commentaire && <p className="text-xs italic text-[#5c584d] mt-1">« {item.commentaire} »</p>}
        </div>
        <div className="text-right shrink-0">
          <p className="mono-font font-bold text-[#1C2321]">{formatMontant(item.montant_ttc)}</p>
          <p className="text-[10px] text-[#8b8677]">TVA {item.tva}</p>
        </div>
      </div>

      <div className="flex items-center flex-wrap gap-3 mt-2 pt-2 border-t border-black/5">
        ...
        <button onClick={onViewAttachments} className="flex items-center gap-1 text-xs text-[#3B6E8F]">
          <Paperclip className="w-3.5 h-3.5" /> Pièces jointes
        </button>
        <div className="flex-1" />
        <button onClick={onEdit} className="p-1.5 rounded hover:bg-black/5"><Pencil className="w-3.5 h-3.5" /></button>
        <button onClick={onDelete} className="p-1.5 rounded hover:bg-black/5 text-[#C1495F]"><Trash2 className="w-3.5 h-3.5" /></button>
      </div>
    </div>
  );
}
